...
const defaultOptions: CorsOptions = {
  ...
  optionsSuccessStatus: 204, // caution: some legacy browsers (IE11, various SmartTVs) choke on 204
  credentials: true,
};
...
function getOriginHeaders(
  ...
  if (allowedOrigins === '*') {
    ...
    const isAllowed =
      reqOrigin &&
      allowedOrigins.some(origin => isOriginAllowed(reqOrigin, origin));
    ...
    allowedOrigins instanceof RegExp
  ) {
    // Single allowed origin (string or regex)
    const isAllowed = reqOrigin && isOriginAllowed(reqOrigin, allowedOrigins);
    if (isAllowed && reqOrigin) {
      headers.set('Access-Control-Allow-Origin', reqOrigin);
    }
    headers.append('Vary', 'Origin');
